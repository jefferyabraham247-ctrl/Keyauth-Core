...
function add($data, $type, $secret = null)
{
        ...
        switch ($type) {
                case 'IP Address':
                        ...
                        cache\purgePattern('AXC_X_GODS_KEYAUTH_Blacklist:' . ($secret ?? $_SESSION['app']) . ':' . $data);
                        break;
                case 'Hardware ID':
        ...
        if ($query->affected_rows > 0) {
                if ($_SESSION['role'] == "seller" || !is_null($secret)) {
                        cache\purge('AXC_X_GODS_KEYAUTH_Blacks:' . ($secret ?? $_SESSION['app']));
                }
                return 'success';
        } else {
                return 'failure';
...
function deleteAll($secret = null)
{
        ...
        if ($query->affected_rows > 0) {
                cache\purgePattern('AXC_X_GODS_KEYAUTH_Blacklist:' . ($secret ?? $_SESSION['app']));
                if ($_SESSION['role'] == "seller" || !is_null($secret)) {
                        cache\purge('AXC_X_GODS_KEYAUTH_Blacks:' . ($secret ?? $_SESSION['app']));
                }
                return 'success';
...
function deleteSingular($blacklist, $type, $secret = null)
{
        ...
        switch ($type) {
                case 'ip':
                        ...
                        cache\purgePattern('AXC_X_GODS_KEYAUTH_Blacklist:' . ($secret ?? $_SESSION['app']) . ':' . $blacklist);
                        break;
                case 'hwid':
...
function addWhite($ip, $secret = null)
{
        ...
        cache\purge('AXC_X_GODS_KEYAUTH_Whitelist:' . ($secret ?? $_SESSION['app']) . ':' . $ip);
