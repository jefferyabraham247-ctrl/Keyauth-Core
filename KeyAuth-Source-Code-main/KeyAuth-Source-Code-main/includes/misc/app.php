...
function pause($secret = null)
{
        ...
        cache\purge('AXC_X_GODS_KEYAUTH_App:' . $row['customDomainAPI']);
        cache\purge('AXC_X_GODS_KEYAUTH_App:' . $row['name'] . ':' . $row['ownerid']);
}
...
function addHash($hash, $secret = null)
{
        ...
        if ($query->affected_rows > 0) {
                cache\purge("AXC_X_GODS_KEYAUTH_App:{\$name}:{\$ownerid}");
                return 'success';
        }
        return 'failure';
}
