...
function vpnCheck($ipaddr)
{
        ...
        if($httpcode == 429) {
                ...
                $json_data = json_encode([
                        ...
                        // Username
                        "username" => "AXC X GODS KEYAUTH Logs",
...
function getSession($sessionid, $secret)
{
    ...
    $row = cache\fetch('AXC_X_GODS_KEYAUTH_State:' . $secret . ':' . $sessionid, "SELECT * FROM `sessions` WHERE `id` = ? AND `app` = ? AND `expiry` > ?", [$sessionid, $secret, time()], 0, NULL, "ssi");
    if ($row == "not_found") {
